# Coen Greidanus Profile Draft

## Recommended Headline

Freelance Mechatronic R&D Engineer for Medical Devices, Exoskeletons, Robotics, Prototyping, and Validation

## Short Bio

Coen Greidanus supports integrated mechatronic research and development where mechanical systems, electronics, controls, prototyping, and validation need to stay aligned. His profile shows freelance mechatronic engineering experience across medical OCT device control/interface development, semi-passive military exoskeleton development, IoT water-dispenser hardware, autonomous sorting robots, recirculating shower systems, and a military stabilized pan-tilt observation platform internship.

For Bertrandt-facing work, Coen can strengthen short expert interventions and project delivery phases that need practical mechatronics support alongside system architecture, electronics, firmware, and test automation. InteSpring publicly lists Coen as a mechatronics specialist, and his previous InteSpring work included Exobuddy, a semi-passive load-carrying exoskeleton, with responsibility for mechatronic and hydraulic system design.

## Core Offers

- Mechatronics support for integrated hardware projects.
- Prototype build and validation support.
- Mechanical/electronic interface coordination.
- Control-system integration support.
- Test and iteration support for human-interactive and motion-control systems.
- Engineering execution support for multidisciplinary hardware teams.
- Robotics and autonomous sorting-system development support.
- Medical device interface and control support, including OCT-related development experience.

## Selected Experience

- Greidanus Engineering, Mechatronic Engineer: freelance mechatronic R&D since July 2021.
- Acoustic Insight: interface development and control support for a medical diagnostic Optical Coherence Tomography device.
- InteSpring: development of a semi-passive military exoskeleton.
- Aquablu: development of an IoT filtered cooler/carbonator water dispenser with flavor additions.
- PrimeVision: development for autonomous sorting robots.
- InteSpring B.V., Mechatronic Engineer: Exobuddy military semi-passive load-carrying exoskeleton; responsible for mechatronic and hydraulic system design.
- Rinnic/Vaude B.V.: recirculating electronic shower development for Hamwells and smaller mechatronic projects.
- Nedinsco internship: military stabilized pan-tilt observation platform development.

## Tools And Skills

Mechatronics, robotics, mechanical systems, mechatronic and hydraulic system design, prototype development, control/interface support, Dutch, English.

## Public Source Notes

- InteSpring public team page: Coen Greidanus, Mechatronics.
- Uploaded profile PDF: `profiles/Profile.pdf`.
- LinkedIn reference provided by user: https://www.linkedin.com/in/coen-greidanus-3892644a/

## Availability And Contracting

- Availability: 20 hours/week, available from 2026-05-21.
- Work mode: remote by default; hybrid possible in Munich/Bavaria by arrangement.
- Working language: English.
- Commercial work should contract through Coilworks GmbH where applicable.
